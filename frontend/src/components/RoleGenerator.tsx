...
export default function RoleGenerator() {
  ...
  const [roleYaml, setRoleYaml] = useState<string | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const selectedResources = resources.filter(r => r.selected && r.selectedVerbs.length > 0);
    if (selectedResources.length === 0) {
      setError('Please select at least one resource and its permissions');
      return;
    }

    try {
      const response = await fetch(`${API_BASE_URL}/api/generate-role`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          name,
          namespace,
          permissions: selectedResources.map(r => ({
            resource: r.name,
            apiGroup: r.apiGroup,
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || 'Failed to generate role');
      }

      const data = await response.json();
      setSuccess(true);
      setRoleYaml(data.role);
    } catch (err) {
  ...
  const handleDownload = () => {
    ...
    const blob = new Blob([roleYaml], { type: 'text/yaml' });
            ...
            {success && (
              <div className="bg-green-50 border-l-4 border-green-400 p-4 mb-6 rounded-r">
                <div className="flex">
                  ...
                  <div className="ml-3">
                    <p className="text-sm text-green-700">Role generated successfully!</p>
                  </div>
                </div>
              </div>
            )}
